...
export function ShattabhaLogo({
  ...
  const rawId = useId();
  // Safe string ID without colons just in case
  const uniqueId = rawId.replace(/[^a-zA-Z0-9]/g, '');

  const blueRibbonGradId = `blueRibbonGrad-${uniqueId}`;
  const blueGlowGradId = `blueGlowGrad-${uniqueId}`;
  const blueShadowGradId = `blueShadowGrad-${uniqueId}`;
  const goldRibbonGradId = `goldRibbonGrad-${uniqueId}`;
  const goldGlowGradId = `goldGlowGrad-${uniqueId}`;
  const goldShadowGradId = `goldShadowGrad-${uniqueId}`;
  const logoDepthShadowId = `logoDepthShadow-${uniqueId}`;
  ...
  const svgIcon = (
    ...
    >
      <defs>
        ...
        <linearGradient id={blueRibbonGradId} x1="138" y1="55" x2="58" y2="110" gradientUnits="userSpaceOnUse">
        ...
        <linearGradient id={blueGlowGradId} x1="60" y1="110" x2="138" y2="55" gradientUnits="userSpaceOnUse">
          <stop offset="0%" stopColor="#7DD3FC" stopOpacity="0.8" />
          <stop offset="100%" stopColor="#0369A1" stopOpacity="0" />
        </linearGradient>
        
        {/* Shaded pocket gradient inside the blue ribbon loop */}
        <linearGradient id={blueShadowGradId} x1="58" y1="110" x2="88" y2="125" gradientUnits="userSpaceOnUse">
          <stop offset="0%" stopColor="#0B1E43" />
          <stop offset="100%" stopColor="#1E40AF" stopOpacity="0.2" />
        </linearGradient>

        {/* Symmetrical cadmium-amber gold gradient for the bottom ribbon */}
        <linearGradient id={goldRibbonGradId} x1="62" y1="145" x2="142" y2="90" gradientUnits="userSpaceOnUse">
